...
const SyncEngine = (() => {
  ...
  // If a save arrives while a push is already in flight, pushAll() used to
  // just `return` and silently drop it — the edit was never retried, so it
  // sat in IndexedDB forever until the next unrelated save happened to push
  // the whole DB again. Worse, the 20s pullAll() timer doesn't know a push
  // is in flight either: it can fetch from Supabase (still showing the OLD
  // value, since the in-flight push hasn't committed yet) and do its usual
  // full local replace, stomping the just-edited value back to the old one
  // right in front of the user — this is Bug: "edited credit amount reverts
  // to old amount as soon as sync runs" (reported Aug 18, 2026). Fixed by
  // (a) queueing any save that arrives mid-push instead of dropping it, and
  // (b) having pullAll() skip its cycle entirely while a push is in flight
  // or queued, so it never overwrites local with data older than what's
  // about to be confirmed on the server.
  let pushQueued = false;
  let pendingPushDb = null;
  let pulledOnce = false; // guards against pushing pre-pull default/seed data
  // Bumped on every local save, BEFORE any async work. Used to detect the
  // case where a pullAll() request was already in flight to Supabase (sent
  // moments before the user's edit) and its response — carrying the OLD
  // value — lands AFTER the edit's push has already committed the new
  // value. Without this check, that stale response still overwrites local
  // state, which is why the "amount reverts" bug could happen on literally
  // any single edit, not just back-to-back ones: it only needs an edit to
  // land inside the ~1-2s window a periodic pull's request/response is in
  // flight, and that 20s timer fires regardless of what the user is doing.
  ...
  /* ---- Serialized push entry point: queues instead of dropping if a push
     is already running, then re-runs itself with the latest db once the
     current push finishes, so no edit made mid-push is ever lost. ---- */
  async function requestPush(db) {
    if (pushInFlight) {
      // Remember the freshest version of the DB to push next — later calls
      // simply replace pendingPushDb rather than piling up duplicate pushes.
      pendingPushDb = db;
      pushQueued = true;
      return;
    }
    ...
    if (pushQueued) {
      pushQueued = false;
      const nextDb = pendingPushDb;
      pendingPushDb = null;
      if (nextDb) await requestPush(nextDb);
    }
  }
  ...
  async function pushAll(db) {
    ...
    if (pushInFlight) return; // safety net; normal callers go through requestPush()
    pushInFlight = true;
    OfflineBanner.hide();
    setStatus('syncing');
    try {
  ...
  async function pullAll() {
    ...
    if (pushInFlight || pushQueued) {
      // A local edit is currently being pushed (or is queued to be pushed).
      // Supabase may not have committed it yet, so a full-replace pull right
      // now would fetch the OLD value and stomp the user's just-made edit
      // back to it on screen. Skip this pull cycle; the 20s timer (or the
      // pull that runs right after requestPush's chain finishes) will catch
      // up once the push has actually landed.
      return;
    }
    setStatus('syncing');
    ...
    try {
      ...
      if (localEditEpoch !== epochAtStart) {
        // The user saved a local edit WHILE this fetch was in flight. This
        // response was requested before that edit existed, so it may still
        // be carrying the pre-edit value even though the edit's own push
        // has (or will) commit separately. Applying it now would silently
        // revert the user's just-made change on screen. Discard this
        // result entirely and retry shortly, once the edit's own push (see
        // requestPush) has had a chance to land on the server.
        setStatus('synced');
        setTimeout(() => { if (!pushInFlight && !pushQueued) pullAll(); }, 2000);
        return;
      }
  ...
  async function notifyLocalChange(db) {
    ...
    localEditEpoch++; // mark "local state changed" before anything async runs
    const beforeIds = JSON.stringify([...(db.accounts||[]), ...(db.categoriesIncome||[]), ...(db.categoriesExpense||[]), ...(db.income||[]), ...(db.expense||[]), ...(db.investments||[])].map(r=>r.id));
    db = migrateLegacyIds(db);
    const afterIds = JSON.stringify([...(db.accounts||[]), ...(db.categoriesIncome||[]), ...(db.categoriesExpense||[]), ...(db.income||[]), ...(db.expense||[]), ...(db.investments||[])].map(r=>r.id));
    if (beforeIds !== afterIds) {
      // migrateLegacyIds() mutates ids in place, but saveDB() already wrote
      // the PRE-migration ids to IndexedDB before calling us. If we don't
      // persist the migrated (UUID) ids here too, the next reload reads the
      // stale legacy id again, generates a DIFFERENT random UUID for it, and
      // re-pushes that as a brand-new row — duplicating categories/accounts
      // (name+kind unique-constraint 23505) and, worse, aborting that same
      // push's transactions upload, so any transaction saved in the same
      // batch silently never reaches Supabase (see README — Bug 19).
      await IDB.set(SYNC_STATE_KEY, db);
    }
    if (!pulledOnce) {
